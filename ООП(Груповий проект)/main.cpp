...
class Settlement: public Region//Населений пункт
{
    string settlementName; //назва
    string geomorphicBinding; //геоморфологічна прив’язка місця відбору проби
...
{
    Settlement settlement;//Населении пункт
    string samplingDate; //дата відбору проби
    double CO2Content; //вміст вуглекислого газу
    double COContent; //вміст СО
    double SO2Content; //вміст SO2
    double GasolineContent; //вміст бензину
    double HClContent; //вміст HCl
...
//Класс для запису даних проб до файлу та його читання
class SampleStorage{
    vector<SamplingData> samples;
public:
    SampleStorage();
    void addSample(); //додати проби
    void deleteSample(); //видалити проби
    void searchSample(); //знати пробу
    void viewSamples(); //відобразити проби
    void fullTable();
private:
    void readSamplesFromFile(); //читати всі проби з файлу
    void writeSamplesToFile(); //запсати всі проби до файлу
    void printSample(int i); //Вивод строки проби до екрану
...
void SampleStorage::fullTable()
{
    ...
    SamplingData sample1(
                        Settlement("Ivano-Frankіvska","Pidkamin","r.Svirzh"),
                        "22.02.97",
    ...
    SamplingData sample2(
                        Settlement("Ivano-Frankіvska","Rogatin","r.Gnila_Lipa"),
                        "22.02.97",
    ...
    SamplingData sample3(
                        Settlement("Chernіvecka","Putila","Vododil"),
                        "22.02.97",
...
int main()
{
    ...
    SampleStorage storage; //сховище для даних проб
